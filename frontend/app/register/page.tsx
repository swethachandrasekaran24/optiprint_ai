...
import { Mail, Lock, User, Loader2, AlertCircle, CheckCircle, Printer } from "lucide-react";

export default function RegisterPage() {
  const router = useRouter();
  const { register, isAuthenticated, loading, error, setError } = useAuthStore();
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 sm:p-10 rounded-3xl shadow-xl shadow-slate-100 border border-slate-100 relative overflow-hidden">
        ...
        <div className="text-center relative z-10">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Join OptiPrint AI and start reducing your carbon and printing footprints.
          </p>
        </div>
        ...
        <div className="text-center relative z-10">
          <p className="text-sm text-slate-500">
            ...
            <Link href="/login" className="font-semibold text-optigreen-600 hover:text-optigreen-700 transition-colors">
              Sign in instead
            </Link>
          </p>
        </div>
